...
from examples.list_keyboard.callbacks import (
    callback_list_dish,
    callback_element_potato,
    callback_element_rice,
)


def generate_button():
    list_group_mashed_potatoes = ListGroup(
        title="Картофельное пюре", subtitle="Not used in docs."
    )
    mashed_potatoes_cutlets = ListGroupItem(
        title="Пюре с котлетками",
        callback_data=callback_element_potato.new(name="cutlets", id="1"),
    )
    mashed_potatoes_chicken = ListGroupItem(
        title="Пюре с курочкой",
        callback_data=callback_element_potato.new(name="chicken", id="2"),
    )

    list_group_rice = ListGroup(title="Рис", subtitle="Not used in docs.")
    rice_cutlets = ListGroupItem(
        title="Рис с котлетками",
        callback_data=callback_element_rice.new(name="cutlets", id="3"),
    )
    rice_chicken = ListGroupItem(
        title="Рис с курочкой",
